...
const PUBLIC_PATHS = ["/", "/login", "/admin/login", "/forgot-password"];
...
function AppShell({ children }) {
  ...
  const isPublic = PUBLIC_PATHS.some((p) => location.pathname.startsWith(p.replace(/\/$/, "")) && p !== "/")
    || location.pathname === "/"
  ...
  return (
    <>
      {/* Mobile top header (only on mobile + authenticated pages) */}
      {showMobileChrome && <MobileTopBar />}

      {/* Page content — push down on mobile for topbar, up for bottom nav */}
      <div style={{
        paddingTop:    showMobileChrome ? 56 : 0,
        paddingBottom: showMobileChrome ? 72 : 0,
        minHeight: "100vh",
        // On mobile authenticated pages: remove any left padding the desktop sidebar adds
