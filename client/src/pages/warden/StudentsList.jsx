...
/* ── Student Row / Card ─────────────────────────────────── */
function StudentCard({ student, onView, isMobile }) {
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 11, color: P.muted, fontFamily: "'DM Mono',monospace", marginTop: 2 }}>
              {student.rollNumber || "—"}
            </div>
          </div>
          ...
          <button
            ...
            style={{
              padding: "7px 14px", background: P.primary, color: "#fff",
        ...
        <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          {student.batch && (
            <span style={{ background: "#F1F5F9", color: P.muted,
...
/* ── Pagination ─────────────────────────────────────────── */
function Pagination({ page, totalPages, total, onPageChange, isMobile }) {
  ...
  return (
    <div style={{ padding:"14px 16px", borderTop:`1px solid ${P.border}`,
      ...
      <span style={{ fontSize:12, color:P.muted }}>
        {isMobile ? `${page}/${totalPages} · ${total} students` : `Page ${page} of ${totalPages} · ${total} students total`}
      </span>
      <div style={{ display:"flex", gap:4 }}>
        ...
        {!isMobile && start>1 && <span style={{ fontSize:12,color:P.muted,padding:"0 2px",alignSelf:"center" }}>…</span>}
        {pages.map(p=>(
          <button key={p} onClick={()=>onPageChange(p)} style={btn(p===page,false)}>{p}</button>
        ))}
        {!isMobile && end<totalPages && <span style={{ fontSize:12,color:P.muted,padding:"0 2px",alignSelf:"center" }}>…</span>}
  ...
  return (
    <div style={{ fontFamily:"'Sora',sans-serif", background:P.surface,
      ...
      <div style={{ display:"flex",
        ...
        <button
          onClick={() => navigate("/warden/add-student")}
      ...
      <div style={{ background:"#fff", borderRadius:16,
        ...
        {[
          ...
          <div key={label} style={{ display:"flex", alignItems:"center", gap:8, flexShrink:0 }}>
            ...
            <div>
              <div style={{ fontSize: isMobile ? 13 : 15, fontWeight:700, color:P.text }}>{value}</div>
              <div style={{ fontSize:10, color:P.muted }}>{label}</div>
            </div>
            <div style={{ width:1, height:28, background:P.border, marginLeft:6 }}/>
          </div>
        ))}
      </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8,
          ...
          {search && (
            <button onClick={()=>setSearch("")}
              style={{ border:"none", background:"none", cursor:"pointer", color:P.muted, fontSize:12 }}>
              ✕
            </button>
          )}
        </div>

        {/* Batch + Department selects on same row for mobile */}
        <div style={{ display:"flex", gap:8,
          ...
          {(batch||department) && (
            <button onClick={()=>handleFilter("","")}
              style={{ padding:"9px 12px", border:`1.5px solid ${P.border}`,
                borderRadius:12, background:"none", color:P.muted,
      ...
      <div style={{ background:"#fff", borderRadius:20,
        ...
        {/* Mobile header */}
        {isMobile && !loading && displayed.length > 0 && (
          <div style={{ padding:"10px 16px", borderBottom:`1px solid ${P.border}`,
            background:"#F8FAFF", fontSize:11, fontWeight:600, color:P.muted,
            textTransform:"uppercase", letterSpacing:"0.06em" }}>
            Students
          </div>
        )}
