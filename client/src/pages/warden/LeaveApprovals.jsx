...
// FIXES:
//  1. Polls every 30s → new student leaves appear automatically, no manual refresh needed
//  2. Re-fetches when browser tab regains focus (coming back from Analytics or other page)
//  3. Re-fetches when switching from Analytics tab → Requests tab
//  4. OPTIMISTIC UI update on Approve/Reject → status changes instantly (0ms delay)
//  5. "New pending" toast banner when polling detects new submissions
...
// ── StatCard ──
function StatCard({ icon, value, label, color, bg, pulse }) {
    ...
    <div style={{ fontFamily: "'Sora',sans-serif" }}>
      ...
      <div style={{ marginTop: 16, padding: "10px 12px", background: "#EFF6FF", borderRadius: 10,
        ...
        <span style={{ width: 10, height: 10, borderRadius: 2, background: "#DBEAFE",
          display: "inline-block", border: `1px solid ${P.primary}` }}/>
...
const LeaveApprovals = () => {
  ...
  // Persist last known pending count to detect new arrivals during polling
  const lastPendingRef   = useRef(null);
  // Always hold latest fetch params so polling uses same filters
  const currentParamsRef = useRef({ page: 1, limit: 20 });

  // ── CORE FETCH ──────────────────────────────────────────────
  // silent=true → background poll, no spinner, no state reset on error
  const doFetch = useCallback(async (params = {}, silent = false) => {
    ...
    try {
      ...
      const pg   = raw?.pagination ?? {
        ...
        page: 1, limit: params.limit ?? 20, totalPages: 1,
      };
      setLeaves(Array.isArray(data) ? data : []);
      setPagination(pg);

      // Detect new pending submissions while warden is on page
      const newPending = Array.isArray(data)
        ? data.filter(l => l.status === "Pending").length
        : (pg.pending ?? 0);
      if (lastPendingRef.current !== null && newPending > lastPendingRef.current && silent) {
        setNewBanner(true);
      }
  ...
  // Convenience: build params from current filter state + overrides
  ...
  // ── 2. RE-FETCH WHEN SWITCHING BACK TO REQUESTS TAB ──
  useEffect(() => {
    if (activeTab === "requests") {
      doFetch(currentParamsRef.current);
    }
  }, [activeTab]); // eslint-disable-line

  // ── 3. AUTO-POLL EVERY 30 SECONDS (silent) ──
  useEffect(() => {
    const id = setInterval(() => {
      if (activeTab === "requests") {
        doFetch(currentParamsRef.current, true /* silent */);
      }
    }, 30_000);
    return () => clearInterval(id);
  }, [doFetch, activeTab]);

  // ── 4. RE-FETCH WHEN BROWSER TAB REGAINS FOCUS ──
  useEffect(() => {
    const handler = () => {
  ...
  // ── 5. OPTIMISTIC APPROVE ──
  // Update UI instantly — user sees "Approved" at once, server confirm runs in background
  const handleApprove = async (id) => {
    setActionLoading(id);
    setLeaves(prev => prev.map(l => l._id === id ? { ...l, status: "Approved" } : l));
    try {
      await updateLeaveStatus(id, "Approved", "");
      doFetch(currentParamsRef.current, true); // silent sync
    } catch (e) {
      setLeaves(prev => prev.map(l => l._id === id ? { ...l, status: "Pending" } : l)); // revert
      alert(e.response?.data?.message || "Approve failed. Please try again.");
  ...
  const handleRejectConfirm = async (id, reason) => {
    ...
    try {
      ...
      alert(e.response?.data?.message || "Reject failed. Please try again.");
    } finally { setActionLoading(null); }
  };

  // Derived live counts (from current leaves array — instant after optimistic update)
  ...
  return (
    <div style={{ fontFamily: "'Sora',sans-serif", background: P.surface, minHeight: "100vh", padding: 24, color: P.text }}>
      <style>{`
        @keyframes la-pulse   { 0%,100%{box-shadow:0 0 0 0 rgba(217,119,6,0.5)}  50%{box-shadow:0 0 0 6px rgba(217,119,6,0)} }
      ...
      {/* Stat Cards — update instantly after approve/reject */}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", gap: 20, alignItems: "start" }}>
            ...
            {[["requests","📋 Leave Requests"],["analytics","📊 Analytics"]].map(([key, lbl]) => (
              <button key={key} onClick={() => setActiveTab(key)}
                ...
                {key === "requests" && livePending > 0 && (
                  <span style={{ marginLeft: 6, background: activeTab === key ? "rgba(255,255,255,0.28)" : P.pending,
                    color: "#fff", fontSize: 10, borderRadius: 20, padding: "1px 7px", fontWeight: 700 }}>
                    {livePending}
                  </span>
                )}
              </button>
                  ...
                  return (
                    <div key={leave._id}
                      style={{ display: "grid", gridTemplateColumns: "2fr 1.3fr 0.7fr 1.1fr 1fr 150px",
                        ...
                        background: isEmg ? "#FFF8F8" : "transparent",
                        opacity: isProc ? 0.65 : 1 }}
                      ...
                      <div>
                        <span style={{ background: sBg, color: sColor, fontSize: 11, fontWeight: 700,
                          ...
                          {isProc && (
                            <span style={{ width: 10, height: 10, border: "2px solid rgba(0,0,0,0.15)",
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              {statsLoading ? (
                ...
                <>
                  <div style={{ background: "#fff", borderRadius: 20, padding: "20px 24px",
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: P.muted, marginTop: 2 }}>Number of leaves applied each month — by status</div>
                      </div>
                      <span style={{ fontSize: 12, color: P.muted, background: "#F1F5F9", padding: "4px 10px", borderRadius: 8 }}>
                        {new Date().getFullYear()}
                      </span>
                    </div>
                    {monthlyChartData.length === 0 ? (
                      <div style={{ height: 220, display: "flex", alignItems: "center", justifyContent: "center", color: P.muted, fontSize: 13 }}>No monthly data yet</div>
                    ) : (
                      <ResponsiveContainer width="100%" height={220}>
                        <AreaChart data={monthlyChartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                          ...
                          <XAxis dataKey="month" tick={{ fontSize: 11, fill: P.muted }} axisLine={false} tickLine={false}/>
                  ...
                  <div style={{ background: "#fff", borderRadius: 20, padding: "20px 24px",
                    border: `1px solid ${P.border}`, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                    <div style={{ fontWeight: 700, fontSize: 15, color: P.text, marginBottom: 4 }}>Department-wise Breakdown</div>
                    <div style={{ fontSize: 12, color: P.muted, marginBottom: 14 }}>Leave counts per department</div>
                    {deptChartData.length === 0 ? (
                      <div style={{ height: 180, display: "flex", alignItems: "center", justifyContent: "center", color: P.muted, fontSize: 13 }}>No data available</div>
                    ) : (
                      <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={deptChartData} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#F0F4FF"/>
                          <XAxis dataKey="dept" tick={{ fontSize: 11, fill: P.muted }} axisLine={false} tickLine={false}/>
                          <YAxis tick={{ fontSize: 11, fill: P.muted }} axisLine={false} tickLine={false}/>
                          <Tooltip content={<ChartTooltip/>}/>
                          <Legend iconType="circle" iconSize={8}/>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                    <div style={{ background: "#fff", borderRadius: 20, padding: "20px 24px",
                      ...
                      {typeChartData.length === 0 ? (
                        <div style={{ height: 200, display: "flex", alignItems: "center", justifyContent: "center", color: P.muted }}>No data yet</div>
                      ) : (
                        <ResponsiveContainer width="100%" height={200}>
                          <PieChart>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ background: `linear-gradient(135deg,${P.primary},#1D4ED8)`,
            ...
            <div style={{ fontSize: 36, fontWeight: 700 }}>{livePending}</div>
