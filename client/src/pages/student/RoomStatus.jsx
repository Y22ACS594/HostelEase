...
const Card = ({children,style={}}) => (
  <div style={{background:"#fff",border:`1px solid ${P.border}`,borderRadius:16,
    padding:"22px 24px",boxShadow:"0 1px 4px rgba(37,99,235,0.06)",...style}}>
    {children}
  </div>
);
      ...
      <div style={{display:"grid",gap:13}}>
        <div>
          ...
          <input name="title" value={form.title} onChange={handle} required
            placeholder="e.g. Leaking tap, broken window lock…"
        ...
        <button type="submit" disabled={submitting}
          ...
          {submitting?(
            <><span style={{width:14,height:14,border:"2px solid rgba(255,255,255,0.35)",
              ...
              animation:"rs-spin .7s linear infinite"}}/>Raising issue…</>
          ):"🔧 Raise Room Issue"}
        </button>
      </div>
    </form>
...
function IssueRow({issue}) {
  ...
  return (
    <div style={{border:`1.5px solid ${open?P.primary:P.border}`,borderRadius:12,
      ...
      boxShadow:open?"0 4px 16px rgba(37,99,235,0.10)":"none",transition:"all .18s"}}>
...
const RoomStatus = () => {
  ...
  useEffect(()=>{
    const boot = async() => {
      try {
        ...
        // ✅ Use dedicated /students/roommates endpoint
        if(roomData?.roomNumber){
          try{
            const rm = await getRoommates();
            setRoommates(rm.roommates ?? []);
          }catch{}
        }
  ...
  if(loading) return (
    <div style={{fontFamily:"'Sora',sans-serif",background:P.surface,minHeight:"100vh",padding:"32px 36px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        {[1,2,3,4].map(i=>(
          <div key={i} style={{background:"#fff",borderRadius:16,padding:22,border:`1px solid ${P.border}`}}>
            ...
            <Skeleton h={22} mb={10}/><Skeleton h={22} w="70%" mb={0}/>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{fontFamily:"'Sora',sans-serif",background:P.surface,minHeight:"100vh",
      padding:"32px 36px",color:P.text}}>
      <style>{`
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              ...
              <Card>
                ...
                {/* ✅ Correct Student model field names */}
                <div>
                ...
                {(profile?.fatherName||profile?.motherName||profile?.parentContact)&&(
                  <div style={{marginTop:16}}>
                    ...
                    <InfoRow label="Parent Phone"  value={profile?.parentContact} />
                  </div>
                )}
              </Card>
            </div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:20}}>
              ...
              <Card>
                ...
                {roommates.length===0 ? (
                  <div style={{textAlign:"center",padding:"28px 20px",color:P.muted}}>
                    ...
                    <div style={{fontSize:12}}>
                      Others will appear here once allocated to Room {room.roomNumber}.
                    </div>
                  </div>
                ):(
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    {roommates.map((mate,i)=>(
                      <div key={mate._id||i} style={{display:"flex",alignItems:"center",gap:12,
                        padding:"12px 14px",background:P.surface,border:`1px solid ${P.border}`,
                        borderRadius:12}}>
                        {mate.avatar ? (
                          <img src={mate.avatar} alt={mate.fullName}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          {mate.batch&&(
                            <div style={{fontSize:10,color:P.muted,marginTop:1}}>Batch: {mate.batch}</div>
                          )}
                        </div>
                        {mate.bedNumber&&(
                ...
                {roommates.length>0&&(
                  <div style={{marginTop:14,padding:"10px 14px",background:"#EFF6FF",
                    borderRadius:10,border:"1px solid #BFDBFE",
                    display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:12,color:P.primary,fontWeight:600}}>
                      🏠 Room {room.roomNumber}
                    </span>
                    <span style={{fontSize:12,color:P.muted}}>
                      {roommates.length+1} residents total (incl. you)
                    </span>
                  </div>
                )}
              </Card>
